...
const validationResult = (schema, data) => {
  ...
  if (result.error) {
    ...
    throw new ValidationError('Validation failed. Please enter all required values correctly', 400, details);
  } else return data;
}
...
const validateRegisterData = data => {
  const schema = Joi.object().keys({
    ...
    password: Joi.string().min(6).pattern(/^(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[0-9]).{6,}$/).required()
      .messages({
